...
def plot_paperFluxes(data1, data2, data3, month_flag, missing_files, out_dir1, out_dir2, out_dir4, obs_temp, foremast, deck7th, ice_station, doy, label1, label2, label3):
    ...
    ax  = fig.add_axes([0.1,0.55,0.5,0.35])   # left, bottom, width, height
    plt.plot(data2['time'], zeros,'--', color='lightgrey')
    ...
    ax  = fig.add_axes([0.1,0.1,0.5,0.35])   # left, bottom, width, height
    # ax = plt.gca()
    plt.plot(data2['time'], zeros,'--', color='lightgrey')
    ...
    sns.distplot(np.squeeze(ice_station['taflux'][indexta]), hist=False, color="darkgrey", kde_kws={"shade": False}, label = 'Ice_station')
    plt.title('sensible_heat_flux [W/m2]')
    plt.legend()
    plt.xlim([-20,60])
    ...
    sns.distplot(np.squeeze(ice_station['lrflux'][indexlr]), hist=False, color="darkgrey", kde_kws={"shade": False})
    plt.title('latent_heat_flux [W/m2]')
    plt.xlim([-20,60])
