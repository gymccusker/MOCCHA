...
def plot_line_TSa(data1, data2, data3, month_flag, missing_files, out_dir1, out_dir2, out_dir4, obs_temp, foremast, deck7th, doy, label1, label2, label3): #, lon, lat):
    ...
    plt.plot(data1['time'], data1['surface_net_SW_radiation'].data, color = 'steelblue')
    plt.plot(data2['time'], data2['surface_net_SW_radiation'].data, color = 'forestgreen')
    if ifs_flag == True:
        plt.plot(data3['time'], data3['sfc_net_sw'].data, color = 'darkorange')
    else:
        plt.plot(data3['time'], data3['surface_net_SW_radiation'].data, color = 'darkorange')
    ...
    if month_flag == -1:
        ...
        if out_dir2[:20] == '5_u-bl661_RA1M_CASIM':
            ...
            fileout = '../FIGS/comparisons/' + out_dir1[:9] + '_' + out_dir2[:9] +'_oden_metum_casim-100_TSa.png'
