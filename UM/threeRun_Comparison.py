...
def inversionIdent(data1, data2, data3, month_flag, missing_files, out_dir1, out_dir2, out_dir4, obs, doy, label1, label2, label3):
    ...
    for i in range(0, np.size(obs['sondes']['doy_drift'])):
        ...
        #### ---------------------------------------------------------------
        ### 1. check for second strongest inversion below invbaseID (index = 1)
        if int(obs['sondes']['thetaE_invbaseID'][i]) > 0:
            print (i)
            obs['sondes']['thetaE_2ndinvID'][i] = np.where(obs['sondes']['thetaE_Diff'][i,:] ==
                np.sort(obs['sondes']['thetaE_Diff'][i,int(obs['sondes']['thetaE_2ndinvID'][i]):27])[::-1][1])[0][0]
        data1['thetaE_2ndinvID'][i] = np.where(data1['thetaE_6hrlyDiff'][i,:] == np.sort(data1['thetaE_6hrlyDiff'][i,int(data1['thetaE_2ndinvID'][i]):27])[::-1][1])[0][0]
        data2['thetaE_2ndinvID'][i] = np.where(data2['thetaE_6hrlyDiff'][i,:] == np.sort(data2['thetaE_6hrlyDiff'][i,int(data2['thetaE_2ndinvID'][i]):27])[::-1][1])[0][0]
        if np.nanmax(data3['thetaE_6hrlyDiff'][i,lt3000]) >= 0.0:
            data3['thetaE_2ndinvID'][i] = np.where(data3['thetaE_6hrlyDiff'][i,:] == np.sort(data3['thetaE_6hrlyDiff'][i,int(data3['thetaE_2ndinvID'][i]):27])[::-1][1])[0][0]

        ### 2. check if second strongest inversion is greater than 2K: if so, label as top of decoupled stable layer
        ...
        ### 3. check if there's a similarly strong inversion at the level below
        for n in range(0,3):
    ...
    for i in range(0, np.size(obs['sondes']['doy_drift'])):
        ...
        plt.plot(np.squeeze(obs['sondes']['thetaE_driftSondes_UM'][i,int(obs['sondes']['thetaE_decoupID'][i])]),
            ...
            '^', markersize = 8, color = 'k', label = 'sonde-interpd decoup surf d$\Theta_{E}$ > ' + str(thresh))
        ...
        if data3['thetaE_decoupID'][i] >= 0.0:     ### ignore nans (missing files)
            plt.plot(np.squeeze(data3['thetaE_6hrly_UM'][i,int(data3['thetaE_decoupID'][i])]),
                ...
                '^', markersize = 8, color = 'darkorange', label = 'ifs-interpd decoup surf d$\Theta_{E}$ > ' + str(thresh))
        ...
        plt.plot(np.squeeze(data1['thetaE_6hrly_UM'][i,int(data1['thetaE_decoupID'][i])]),
            ...
            '^', markersize = 8, color = 'steelblue', label = 'um_ra2m decoup surf d$\Theta_{E}$ > ' + str(thresh))
        ...
        plt.plot(np.squeeze(data2['thetaE_6hrly_UM'][i,int(data2['thetaE_decoupID'][i])]),
            ...
            '^', markersize = 8, color = 'forestgreen', label = 'um_casim-100 decoup surf d$\Theta_{E}$ > ' + str(thresh))
