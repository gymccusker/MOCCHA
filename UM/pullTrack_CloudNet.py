        ...
        for k in range(0,np.size(cube)):            ### loop over number of variables
            ...
            if dim_flag == 1:         ### 4D VARIABLE
                if stream[1:3] == 'pd':
                    ...
                    model_lat = DimCoord(cube[1].dim_coords[2].points, var_name = 'grid_latitude', standard_name = 'grid_latitude', units='')
                    model_lon = DimCoord(cube[1].dim_coords[2].points, var_name = 'grid_longitude', standard_name = 'grid_longitude', units='')
