        ...
        for k in range(0,np.size(cube)):            ### loop over number of variables
            ...
            if dim_flag == 1:         ### 4D VARIABLE
                if stream[1:3] == 'pd':
                    ...
                    model_lat = DimCoord(np.size(ncube,3), var_name = 'y_position', standard_name = 'latitude', units='grid boxes')
                    model_lon = DimCoord(np.size(ncube,4), var_name = 'x_position', standard_name = 'longitude', units='grid boxes')
