...
def gridShipTrack(cube, xoffset, yoffset):
    ...
    # lon17[:] = 254
    # lat17 = np.zeros([np.size(tim17)])
    # lat17[:] = 469
    # tim_148 = np.append(tim_148, tim17)
    # lat_148 = np.append(lat_148, lat17)
    # lon_148 = np.append(lon_148, lon17)
    # for i in range(0,np.size(lon11)):
    #     iplt.scatter(cube.dim_coords[2][int(lon11[i]) + xoffset], cube.dim_coords[1][int(lat11[i]) + yoffset],color='black')
...
def trackShip(data):
    ...
    ## DEFINE METUM PERIOD (CLOUDNET COMPARISON)
    ###################################
    trackShip_start = np.where(np.logical_and(np.logical_and(data.values[:,2]==14,data.values[:,1]==8),data.values[:,3]>=16))
    trackShip_end = np.where(np.logical_and(np.logical_and(data.values[:,2]==14,data.values[:,1]==8),data.values[:,3]==18))
...
def plot_cartmap(ship_data, cube, hour): #, lon, lat):
    ...
    ### set size
    ax.set_extent([0, 60, 89.4, 89.7], crs=ccrs.PlateCarree())       ### ZOOM
    ...
    qplt.outline(cube[hour,460:495,240:264])          ### 12-13th Aug swath
