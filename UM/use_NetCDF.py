...
def trackShip(data):
    ...
    trackShip_end = np.where(np.logical_and(np.logical_and(data.values[:,2]==14,data.values[:,1]==8),data.values[:,3]==2))
...
def plot_cartmap(ship_data, cube, hour): #, lon, lat):
    ...
    ### set size
    ax.set_extent([0, 60, 89.2, 90], crs=ccrs.PlateCarree())       ### ZOOM
