...
def trackShip(data):
    ...
    trackShip_end = np.where(np.logical_and(np.logical_and(data.values[:,2]==14,data.values[:,1]==8),data.values[:,3]==4))
...
def plot_cartmap(ship_data, cube, hour): #, lon, lat):
    ...
    ### set size
    ax.set_extent([0, 60, 89.4, 89.8], crs=ccrs.PlateCarree())       ### ZOOM
