...
def splitShip(data, date):
    ...
    Sep1_drift_index = np.where(np.logical_and(np.logical_and(data.values[:,2]<=4,data.values[:,1]==9),data.values[:,3]<=1))
    splitShip1_index = range(Aug1_drift_index[0][0],Sep1_drift_index[0][-1])

    Aug2_drift_index = np.where(np.logical_and(np.logical_and(data.values[:,2]>=4,data.values[:,1]==9),data.values[:,3]>1))
