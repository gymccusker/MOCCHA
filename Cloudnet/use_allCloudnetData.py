...
def plot_CvProfiles(um_data, ifs_data, misc_data, ra2t_data, obs_data, month_flag, missing_files, um_out_dir, doy, obs, obs_switch, obs_testing_flag): #, lon, lat):
    ...
    if obs_testing_flag != 1: ifs_data['model_snow_Cv_filtered'][ifs_data['model_snow_Cv_filtered'] < 0.0] = np.nan
    ...
    if obs_testing_flag != 1:
        plt.plot(np.nanmean(ifs_data['model_snow_Cv_filtered'],0),np.nanmean(ifs_data['height'],0), color = '#FFC107', linewidth = 3, label = 'ECMWF_IFS', zorder = 4)
    elif obs_testing_flag == 1:
        plt.plot(np.nanmean(ifs_data['Cv'],0),np.nanmean(ifs_data['height'],0), color = 'grey', linewidth = 3, label = 'Obs_Ceilometer', zorder = 4)
    plt.plot(np.nanmean(misc_data['model_Cv_filtered'],0),np.nanmean(misc_data['height'],0), color = 'mediumseagreen', linewidth = 3, label = 'UM_CASIM-100', zorder = 3)
    ...
    print ('UM_CASIM-100 = ')
    print (np.nanmean(np.nanmean(misc_data['model_Cv_filtered'][:,Zindex_um[0]],0)))
    print ('UM_RA2M = ')
    print (np.nanmean(np.nanmean(um_data['model_Cv_filtered'][:,Zindex_um[0]],0)))
    print ('UM_RA2T = ')
    print (np.nanmean(np.nanmean(ra2t_data['model_Cv_filtered'][:,Zindex_um[0]],0)))
    print ('Obs = ')
    print (np.nanmean(np.nanmean(obs_data['Cv'][:,Zindex_um[0]],0)))
    ...
    #     '--', color = 'darkblue', linewidth = 0.5)
    #
    # plt.plot(np.nanmedian(ifs_data['model_snow_Cv_filtered'],0),np.nanmedian(ifs_data['height'],0), color = '#FFC107', linewidth = 3, label = 'ECMWF_IFS', zorder = 4)
...
def plot_CvTimeseries(um_data, ifs_data, misc_data, ra2t_data, obs_data, month_flag, missing_files, um_out_dir, doy, obs_switch, obs, data1, data2, data3, data4, obs_testing_flag):
    ...
    for file in missing_files:
        ax.fill_between(np.arange(file, file + 1, 1/24.0), nans[0], nans[-1],
            ...
            zorder = 2)
    ax2 = ax.twinx()
    if obs_testing_flag != 1: ax2.set_ylabel('Measurements \n (1 hour sampling)', rotation = 270, labelpad = 35)
    if obs_testing_flag == 1: ax2.set_ylabel('(Lidar) Measurements \n (1 hour sampling)', rotation = 270, labelpad = 35)
    ax2.set_yticks([])
    ...
    if obs_testing_flag != 1:
        ...
        plt.contourf(ifs_data['time'], np.squeeze(ifs_data['height'][0,:]), np.transpose(ifs_data['Cv']),
            np.arange(0,1.1,0.1),
            cmap = newcmp,
            zorder = 1)
    # plt.plot(np.squeeze(obs['inversions']['doy']),np.squeeze(obs['inversions']['invbase']), 'k', linewidth = 1.0)
    ...
    if month_flag == -1:
        ...
        # fileout = 'FIGS/Obs-UMGrid-qf70-Ceilometer_RA2M-25_CASIM-100-GA6alb_RA2T_CvTimeseries_226-250DOY_lt500m.png'
    plt.savefig(fileout)
    plt.show()
#
def plot_lwcProfiles(um_data, ifs_data, misc_data, ra2t_data, obs_data, month_flag, missing_files, um_out_dir, doy, obs_switch): #, lon, lat):
    ...
    # obs_data['lwc_adiabatic_inc_nolwp_inc_nolwp'][obs_data['lwc_adiabatic_inc_nolwp_inc_nolwp'] == -999] = 0.0
    ...
    if obs_switch == 'RADAR':
        ...
        ax1.fill_betweenx(np.nanmean(obs_data['height'][:,lwc_binlimit:],0),np.nanmean(obs_data['lwc_adiabatic_inc_nolwp'][:,lwc_binlimit:],0)*1e3 - np.nanstd(obs_data['lwc_adiabatic_inc_nolwp'][:,lwc_binlimit:],0)*1e3,
            np.nanmean(obs_data['lwc_adiabatic_inc_nolwp'][:,lwc_binlimit:],0)*1e3 + np.nanstd(obs_data['lwc_adiabatic_inc_nolwp'][:,lwc_binlimit:],0)*1e3, color = 'lightgrey', alpha = 0.5)
        # plt.xlim([0,0.2])
    ...
    plt.plot(np.nanmean(ifs_data['model_lwc'],0)*1e3,np.nanmean(ifs_data['height'],0), color = '#FFC107', linewidth = 3, label = 'ECMWF_IFS', zorder = 4)
    ...
    #     # plt.plot(np.nanmean(obs_data['lwc_adiabatic_inc_nolwp'],0)*1e3,np.nanmean(obs_data['height'],0), color = 'k', linewidth = 3, label = 'Obs_' + obs_switch + 'grid-adiabatic', zorder = 5)
    #     plt.plot(np.nanmedian(obs_data['lwc_adiabatic_inc_nolwp'],0)*1e3,np.nanmedian(obs_data['height'],0), color = 'k', linewidth = 3, label = 'Obs_' + obs_switch + 'grid-adiabatic', zorder = 5)
    #     ax1.fill_betweenx(np.nanmedian(obs_data['height'],0),np.nanmedian(obs_data['lwc_adiabatic_inc_nolwp'],0)*1e3 - np.nanstd(obs_data['lwc'],0)*1e3,
    #         np.nanmedian(obs_data['lwc_adiabatic_inc_nolwp'],0)*1e3 + np.nanstd(obs_data['lwc_adiabatic_inc_nolwp'],0)*1e3, color = 'lightgrey', alpha = 0.5)
    #     # plt.xlim([0,0.2])
    #     plt.plot(np.nanmedian(obs_data['lwc_adiabatic_inc_nolwp'],0)*1e3 - np.nanstd(obs_data['lwc_adiabatic_inc_nolwp'],0)*1e3, np.nanmedian(obs_data['height'],0),
    #         '--', color = 'k', linewidth = 0.5)
    #     plt.plot(np.nanmedian(obs_data['lwc_adiabatic_inc_nolwp'],0)*1e3 + np.nanstd(obs_data['lwc_adiabatic_inc_nolwp'],0)*1e3, np.nanmedian(obs_data['height'],0),
    #         '--', color = 'k', linewidth = 0.5)
    #     # #### ADIABATIC LWC (all times)
    #     # plt.plot(np.nanmean(obs_data['lwc_adiabatic_inc_nolwp_inc_nolwp'],0)*1e3,np.nanmean(obs_data['height'],0), color = 'purple', linewidth = 3, label = 'Obs_' + obs_switch + 'grid-adiabatic-incNoLWP', zorder = 5)
    #     # ax1.fill_betweenx(np.nanmean(obs_data['height'],0),np.nanmean(obs_data['lwc_adiabatic_inc_nolwp_inc_nolwp'],0)*1e3 - np.nanstd(obs_data['lwc'],0)*1e3,
    #     #     np.nanmean(obs_data['lwc_adiabatic_inc_nolwp_inc_nolwp'],0)*1e3 + np.nanstd(obs_data['lwc_adiabatic_inc_nolwp_inc_nolwp'],0)*1e3, color = 'lightgrey', alpha = 0.5)
    #     # plt.xlim([0,0.2])
    #     # plt.plot(np.nanmean(obs_data['lwc_adiabatic_inc_nolwp_inc_nolwp'],0)*1e3 - np.nanstd(obs_data['lwc_adiabatic_inc_nolwp_inc_nolwp'],0)*1e3, np.nanmean(obs_data['height'],0),
    #     #     '--', color = 'k', linewidth = 0.5)
    #     # plt.plot(np.nanmean(obs_data['lwc_adiabatic_inc_nolwp_inc_nolwp'],0)*1e3 + np.nanstd(obs_data['lwc_adiabatic_inc_nolwp_inc_nolwp'],0)*1e3, np.nanmean(obs_data['height'],0),
    #     #     '--', color = 'k', linewidth = 0.5)
    #
    ...
    #     '--', color = 'darkblue', linewidth = 0.5)
    #
    # plt.plot(np.nanmedian(ifs_data['model_lwc'],0)*1e3,np.nanmedian(ifs_data['height'],0), color = '#FFC107', linewidth = 3, label = 'ECMWF_IFS', zorder = 4)
    ...
    print ('Obs = ')
    print (np.nanmean(obs_data['lwc_adiabatic_inc_nolwp'][:,Zindex1[0]],0)*1e3)
...
def plot_LWCTimeseries(um_data, ifs_data, misc_data, obs_data, month_flag, missing_files, um_out_dir, doy, obs_switch): #, lon, lat):
    ...
    if obs_switch == 'RADAR':
        ...
        plt.pcolor(obs_data['time'], np.squeeze(obs_data['height'][0,:]), np.transpose(obs_data['lwc_adiabatic_inc_nolwp'])*1e3,
            vmin = 0.0, vmax = cmax)
            #cmap = newcmp)
    plt.ylabel('Height [m]')
    plt.ylim([0,5000])
...
def plot_iwcProfiles(um_data, ifs_data, misc_data, ra2t_data, obs_data, month_flag, missing_files, um_out_dir, doy, obs_switch): #, lon, lat):
    ...
    plt.plot(np.nanmean(ifs_data['model_snow_iwc_filtered'],0)*1e3 - np.nanstd(ifs_data['model_snow_iwc_filtered'],0)*1e3, np.nanmean(ifs_data['height'],0),
        '--', color = '#FFC107', linewidth = 0.5, zorder = 3)
    plt.plot(np.nanmean(ifs_data['model_snow_iwc_filtered'],0)*1e3 + np.nanstd(ifs_data['model_snow_iwc_filtered'],0)*1e3, np.nanmean(ifs_data['height'],0),
        '--', color = '#FFC107', linewidth = 0.5)
    ...
    plt.plot(np.nanmean(ifs_data['model_snow_iwc_filtered'],0)*1e3,np.nanmean(ifs_data['height'],0), color = '#FFC107', linewidth = 3, label = 'ECMWF_IFS', zorder = 4)
    ...
    #     '--', color = 'darkblue', linewidth = 0.5)
    #
    # plt.plot(np.nanmedian(ifs_data['model_snow_iwc_filtered'],0)*1e3,np.nanmedian(ifs_data['height'],0), color = '#FFC107', linewidth = 3, label = 'ECMWF_IFS', zorder = 4)
...
def plot_twcProfiles(um_data, ifs_data, misc_data, ra2t_data, obs_data, month_flag, missing_files, um_out_dir, doy, obs_switch): #, lon, lat):
    ...
    plt.plot(np.nanmean(ifs_data['model_twc'],0)*1e3,np.nanmean(ifs_data['height'],0), color = '#FFC107', linewidth = 3, label = 'ECMWF_IFS', zorder = 4)
...
def plot_TWCTimeseries(um_data, ifs_data, misc_data, ra2t_data, obs_data, month_flag, missing_files, um_out_dir, doy, obs_switch, obs, data1, data2, data3, data4, nanind, lwcind, iwcind):
    ...
    plt.plot(np.nanmean(mask3,0),np.nanmean(ifs_data['height'],0), color = '#FFC107', linewidth = 3, label = 'ECMWF_IFS', zorder = 4)
    ...
    plt.plot(np.nanmean(obs_data['twc'],0),np.nanmean(obs_data['height'],0), 'k', linewidth = 3, label = 'Obs-' + obs_switch + 'grid', zorder = 5)
    ax1.fill_betweenx(np.nanmean(obs_data['height'],0),np.nanmean(obs_data['twc'],0) - np.nanstd(obs_data['twc'],0),
        np.nanmean(obs_data['twc'],0) + np.nanstd(obs_data['twc'],0), color = 'lightgrey', alpha = 0.5)
    plt.plot(np.nanmean(obs_data['twc'],0) - np.nanstd(obs_data['twc'],0), np.nanmean(obs_data['height'],0),
        '--', color = 'k', linewidth = 0.5)
    plt.plot(np.nanmean(obs_data['twc'],0) + np.nanstd(obs_data['twc'],0), np.nanmean(obs_data['height'],0),
        '--', color = 'k', linewidth = 0.5)

    ax1.fill_betweenx(np.nanmean(ifs_data['height'],0),np.nanmean(ifs_data['model_twc'],0) - np.nanstd(ifs_data['model_twc'],0),
        np.nanmean(ifs_data['model_twc'],0) + np.nanstd(ifs_data['model_twc'],0), color = 'navajowhite', alpha = 0.35)
    plt.plot(np.nanmean(ifs_data['model_twc'],0) - np.nanstd(ifs_data['model_twc'],0), np.nanmean(ifs_data['height'],0),
        '--', color = '#FFC107', linewidth = 0.5)
    plt.plot(np.nanmean(ifs_data['model_twc'],0) + np.nanstd(ifs_data['model_twc'],0), np.nanmean(ifs_data['height'],0),
        '--', color = '#FFC107', linewidth = 0.5)

    ax1.fill_betweenx(np.nanmean(um_data['height'],0),np.nanmean(um_data['model_twc'],0) - np.nanstd(um_data['model_twc'],0),
        np.nanmean(um_data['model_twc'],0) + np.nanstd(um_data['model_twc'],0), color = 'blue', alpha = 0.05)
    plt.plot(np.nanmean(um_data['model_twc'],0) - np.nanstd(um_data['model_twc'],0), np.nanmean(um_data['height'],0),
        '--', color = 'darkblue', linewidth = 0.5)
    plt.plot(np.nanmean(um_data['model_twc'],0) + np.nanstd(um_data['model_twc'],0), np.nanmean(um_data['height'],0),
        '--', color = 'darkblue', linewidth = 0.5)

    ax1.fill_betweenx(np.nanmean(misc_data['height'],0),np.nanmean(misc_data['model_twc'],0) - np.nanstd(misc_data['model_twc'],0),
        np.nanmean(misc_data['model_twc'],0) + np.nanstd(misc_data['model_twc'],0), color = 'mediumaquamarine', alpha = 0.15)
    plt.plot(np.nanmean(misc_data['model_twc'],0) - np.nanstd(misc_data['model_twc'],0), np.nanmean(misc_data['height'],0),
        '--', color = 'mediumseagreen', linewidth = 0.5)
    plt.plot(np.nanmean(misc_data['model_twc'],0) + np.nanstd(misc_data['model_twc'],0), np.nanmean(misc_data['height'],0),
        '--', color = 'mediumseagreen', linewidth = 0.5)

    ax1.fill_betweenx(np.nanmean(ra2t_data['height'],0),np.nanmean(ra2t_data['model_twc'],0) - np.nanstd(ra2t_data['model_twc'],0),
        np.nanmean(ra2t_data['model_twc'],0) + np.nanstd(ra2t_data['model_twc'],0), color = 'lightblue', alpha = 0.15)
    plt.plot(np.nanmean(ra2t_data['model_twc'],0) - np.nanstd(ra2t_data['model_twc'],0), np.nanmean(ra2t_data['height'],0),
        '--', color = 'steelblue', linewidth = 0.5)
    plt.plot(np.nanmean(ra2t_data['model_twc'],0) + np.nanstd(ra2t_data['model_twc'],0), np.nanmean(ra2t_data['height'],0),
        '--', color = 'steelblue', linewidth = 0.5)

    plt.plot(np.nanmean(ifs_data['model_twc'],0),np.nanmean(ifs_data['height'],0), color = '#FFC107', linewidth = 3, label = 'ECMWF_IFS', zorder = 4)
    plt.plot(np.nanmean(misc_data['model_twc'],0),np.nanmean(misc_data['height'],0), color = 'mediumseagreen', linewidth = 3, label = 'UM_CASIM-100', zorder = 3)
    plt.plot(np.nanmean(ra2t_data['model_twc'],0),np.nanmean(ra2t_data['height'],0), color = 'steelblue', linewidth = 3, label = 'UM_RA2T', zorder = 2)
    plt.plot(np.nanmean(um_data['model_twc'],0),np.nanmean(um_data['height'],0), color = 'darkblue', linewidth = 3, label = 'UM_RA2M', zorder = 1)
...
def plot_LWP(um_data, ifs_data, misc_data, ra2t_data, obs_data, obs, month_flag, missing_files, um_out_dir, doy, obs_switch):#,lwp): #, lon, lat):
    ...
    plt.plot(ifs_data['time'][::res],ifs_data['model_lwp'][::res]*1e3,
        'v', color = '#FFC107', markeredgecolor = 'orange', label = 'ECMWF_IFS')
    ...
    sns.distplot(ifs_data['model_lwp']*1e3, hist=False, color="#FFC107", kde_kws={"shade": True})
...
def plot_BiVAR(um_data, ifs_data, misc_data, ra2t_data, obs_data, obs, month_flag, missing_files, cn_um_out_dir, doy, obs_switch, data1, data2, data3, data4, nanind, lwcind, iwcind, out_dir1):
    ...
    # plt.plot(data3['time_hrly'][:-3], data3['iwv_hrly'][:-3] - drift_iwv_1hr[:-2], color = '#FFC107')
    ...
    for bias in bias_list:
        for argx in argx_list:
            ...
            if argx == 'LWP':
                ...
                data1['biases'][argx] = np.nanmean(mask1[:-3,lt3km_um[0]],1) - np.nanmean(mask0[:-3,lt3km_um[0]],1)
            elif argx == 'Cv3km':
                data1['biases'][argx] = np.nanmean(um_data['model_Cv_filtered'][:-3,lt3km_um[0]],1) - np.nanmean(obs_data['Cv'][:-3,lt3km_um[0]],1)
            ...
            if argx == 'LWP':
                ...
                data2['biases'][argx] = np.nanmean(mask2[:-3,lt3km_um[0]],1) - np.nanmean(mask0[:-3,lt3km_um[0]],1)
            elif argx == 'Cv3km':
                data2['biases'][argx] = np.nanmean(misc_data['model_Cv_filtered'][:-3,lt3km_um[0]],1) - np.nanmean(obs_data['Cv'][:-3,lt3km_um[0]],1)
            ...
            if argx == 'LWP':
                ...
                data3['biases'][argx] = np.nanmean(mask3[:-3,lt3km_ifs[0]],1) - np.nanmean(mask0[:-3,lt3km_um[0]],1)
            elif argx == 'Cv3km':
                data3['biases'][argx] = np.nanmean(ifs_data['model_snow_Cv_filtered'][:-3,lt3km_ifs[0]],1) - np.nanmean(obs_data['Cv'][:-3,lt3km_um[0]],1)
            ...
            if argx == 'LWP':
                ...
                data4['biases'][argx] = np.nanmean(mask4[:-3,lt3km_um[0]],1) - np.nanmean(mask0[:-3,lt3km_um[0]],1)
            elif argx == 'Cv3km':
                data4['biases'][argx] = np.nanmean(ra2t_data['model_Cv_filtered'][:-3,lt3km_um[0]],1) - np.nanmean(obs_data['Cv'][:-3,lt3km_um[0]],1)
    ...
    # plt.plot(ifs_data['model_lwp'][:-3]*1e3,data3['fixed_radiation']['SWd'], '.', alpha = 0.5, color = '#FFC107', label = 'ECMWF_IFS')
    # plt.legend()
    # plt.xlabel('LWP [g m$^{-2}$]')
    ...
    # plt.plot(ifs_data['model_lwp'][:-3]*1e3,data3['fixed_radiation']['LWd'], '.', alpha = 0.5, color = '#FFC107')
    # plt.xlabel('LWP [g m$^{-2}$]')
    ...
    # plt.plot(ifs_data['model_lwp'][:-3]*1e3,data3['fixed_radiation']['Rnet'], '.', alpha = 0.5, color = '#FFC107')
    # plt.xlabel('LWP [g m$^{-2}$]')
    ...
    # plt.plot(data3['biases']['LWP'], data3['biases']['SWd'], '.', alpha = alf, color = '#FFC107')
    ...
    # plt.plot(data3['biases']['LWP'], data3['biases']['SWd-LWP_regres']['line'], color = '#FFC107')
    ...
    # plt.plot(data3['biases']['LWP'], data3['biases']['LWd'], '.', alpha = alf, color = '#FFC107')
    ...
    # plt.plot(data3['biases']['LWP'], data3['biases']['LWd-LWP_regres']['line'], color = '#FFC107')
    ...
    # plt.plot(data3['biases']['LWP'], data3['biases']['Rnet'], '.', alpha = alf, color = '#FFC107')
    ...
    # plt.plot(data3['biases']['LWP'], data3['biases']['Rnet-LWP_regres']['line'], color = '#FFC107')
    ...
    ##################################################
    ### -------------------------------
    ### Build simple radiation-IWV bias figures
    ### -------------------------------
    # SMALL_SIZE = 12
    # MED_SIZE = 16
    # LARGE_SIZE = 16
    ...
    # y1min = -130
    # y1max = 80
    # plt.plot(data1['biases'][arg], data1['biases']['SWd'], '.', alpha = alf, color = 'darkblue')
    # plt.plot(data2['biases'][arg], data2['biases']['SWd'], '.', alpha = alf, color = 'mediumseagreen')
    # plt.plot(data3['biases'][arg], data3['biases']['SWd'], '.', alpha = alf, color = '#FFC107')
    # plt.plot(data4['biases'][arg], data4['biases']['SWd'], '.', alpha = alf, color = 'steelblue')
    # plt.plot(data1['biases'][arg], data1['biases']['SWd-' + arg + '_regres']['line'], color = 'darkblue')
    # plt.plot(data2['biases'][arg], data2['biases']['SWd-' + arg + '_regres']['line'], color = 'mediumseagreen')
    # plt.plot(data3['biases'][arg], data3['biases']['SWd-' + arg + '_regres']['line'], color = '#FFC107')
    # plt.plot(data4['biases'][arg], data4['biases']['SWd-' + arg + '_regres']['line'], color = 'steelblue')
    # # plt.grid('on')
    # plt.ylim([y1min,y1max])
    # plt.xlim([xmin,xmax])
    # plt.plot([xmin,xmax],[0,0],'--', color='lightgrey', linewidth = 1, zorder=0)
    # plt.plot([0,0],[y1min,y1max],'--', color='lightgrey', linewidth = 1, zorder=0)
    # plt.xlabel(arg + ' [mod-obs]')
    # plt.ylabel('SW$_{\downarrow}$ [mod-obs]')
    #
    # plt.subplot(132)
    # y2min = -80
    # y2max = 90
    # plt.plot(data1['biases'][arg], data1['biases']['LWd'], '.', alpha = alf, color = 'darkblue')
    # plt.plot(data2['biases'][arg], data2['biases']['LWd'], '.', alpha = alf, color = 'mediumseagreen')
    # plt.plot(data3['biases'][arg], data3['biases']['LWd'], '.', alpha = alf, color = '#FFC107')
    # plt.plot(data4['biases'][arg], data4['biases']['LWd'], '.', alpha = alf, color = 'steelblue')
    # plt.plot(data1['biases'][arg], data1['biases']['LWd-' + arg + '_regres']['line'], color = 'darkblue')
    # plt.plot(data2['biases'][arg], data2['biases']['LWd-' + arg + '_regres']['line'], color = 'mediumseagreen')
    # plt.plot(data3['biases'][arg], data3['biases']['LWd-' + arg + '_regres']['line'], color = '#FFC107')
    # plt.plot(data4['biases'][arg], data4['biases']['LWd-' + arg + '_regres']['line'], color = 'steelblue')
    # # plt.grid('on')
    # plt.ylim([y2min,y2max])
    # plt.xlim([xmin,xmax])
    # plt.plot([xmin,xmax],[0,0],'--', color='lightgrey', linewidth = 1, zorder=0)
    ...
    # y3min = -80
    # y3max = 120
    # plt.plot(data1['biases'][arg], data1['biases']['Rnet'], '.', alpha = alf, color = 'darkblue')
    # plt.plot(data2['biases'][arg], data2['biases']['Rnet'], '.', alpha = alf, color = 'mediumseagreen')
    # plt.plot(data3['biases'][arg], data3['biases']['Rnet'], '.', alpha = alf, color = '#FFC107')
    # plt.plot(data4['biases'][arg], data4['biases']['Rnet'], '.', alpha = alf, color = 'steelblue')
    # plt.plot(data1['biases'][arg], data1['biases']['Rnet-' + arg + '_regres']['line'], color = 'darkblue')
    # plt.plot(data2['biases'][arg], data2['biases']['Rnet-' + arg + '_regres']['line'], color = 'mediumseagreen')
    # plt.plot(data3['biases'][arg], data3['biases']['Rnet-' + arg + '_regres']['line'], color = '#FFC107')
    # plt.plot(data4['biases'][arg], data4['biases']['Rnet-' + arg + '_regres']['line'], color = 'steelblue')
    # # plt.grid('on')
    # plt.ylim([y2min,y2max])
    # plt.xlim([xmin,xmax])
    # plt.plot([xmin,xmax],[0,0],'--', color='lightgrey', linewidth = 1, zorder=0)
    ...
    ##################################################
    ### -------------------------------
    ### Build radiation-LWP-IWV bias figures
    ### -------------------------------
    ##################################################
    ##################################################
    ...
    lwc_biases = [obs_data['lwc_adiabatic_inc_nolwp'][:-3,:]*1e3, um_data['model_lwc'][:-3,:]*1e3 - obs_data['lwc_adiabatic_inc_nolwp'][:-3,:]*1e3,
        misc_data['model_lwc'][:-3,:]*1e3 - obs_data['lwc_adiabatic_inc_nolwp'][:-3,:]*1e3, ifs_data['model_lwc'][:-3,:]*1e3,
        ra2t_data['model_lwc'][:-3,:]*1e3 - obs_data['lwc_adiabatic_inc_nolwp'][:-3,:]*1e3]
    ...
    cloud_mask_bias_lt3km = [mask0[:-3,lt3km_um[0]], data1['biases']['Mask3km'], data2['biases']['Mask3km'], data3['biases']['Mask3km'], data4['biases']['Mask3km']]
    Cv_bias_lt3km = [obs_data['Cv'][:-3,lt3km_um[0]], data1['biases']['Cv3km'], data2['biases']['Cv3km'], data3['biases']['Cv3km'], data4['biases']['Cv3km']]
    ...
    cmap_choice = mpl_cm.coolwarm

    # offset = mcolors.TwoSlopeNorm(vmin=vminn, vcenter=0., vmax=vmaxx)

    ### choose variables and plot arguments for figure
    var = Cv_bias_lt3km
    arg = [var[0], var[1], var[2], var[3], var[4]]    ## 1D timeseries data
    # arg = [var[0], offset(var[1]), offset(var[2]), offset(var[3]), offset(var[4])]    ## 1D timeseries data (offset, diverging with midpoint 0)
    ...
    plt.plot(data3['biases']['LWP'], data3['biases']['SWd-LWP_regres']['line'], color = '#FFC107', zorder = 3)
    plt.ylim([y1min,y1max])
    ...
    plt.plot(data3['biases']['LWP'], data3['biases']['LWd-LWP_regres']['line'], color = '#FFC107', zorder = 3)
    plt.ylim([y2min,y2max])
    plt.xlim([xmin,xmax])
    plt.plot([xmin,xmax],[0,0],'--', color='lightgrey', linewidth = 1, zorder=0)
    plt.plot([0,0],[y2min,y2max],'--', color='lightgrey', linewidth = 1, zorder=0)
    ...
    plt.plot(data3['biases']['LWP'], data3['biases']['Rnet-LWP_regres']['line'], color = '#FFC107', zorder = 3)
    ...
    cbaxes.set_xlabel('C$_{V}$ bias \n(mean below 3km)', rotation = 0, labelpad = 5, fontsize = 12)
    # cbaxes.xaxis.set_ticks([0, 1])
...
def plot_ObsGridComparison(um_data, ifs_data, misc_data, ra2t_data, obs_data, month_flag, missing_files, um_out_dir, doy): #, lon, lat):
    ...
    plt.plot(np.nanmean(ifs_data['Cv'],0),np.nanmean(ifs_data['height'],0), color = '#FFC107', linewidth = 3, label = 'Obs_IFS')
...
def plot_CvProfiles_SplitSeason(um_data, ifs_data, misc_data, obs_data, month_flag, missing_files, um_out_dir, doy): #, lon, lat):
    ...
    plt.plot(np.nanmean(np.squeeze(ifs_data['model_snow_Cv_filtered'][melt,:]),0),np.nanmean(np.squeeze(ifs_data['height'][melt,:]),0), color = '#FFC107', linewidth = 3, label = 'ECMWF_IFS')
    ...
    plt.plot(np.nanmean(np.squeeze(ifs_data['model_snow_Cv_filtered'][freeze,:]),0),np.nanmean(np.squeeze(ifs_data['height'][freeze,:]),0), color = '#FFC107', linewidth = 3, label = 'ECMWF_IFS')
...
def plot_lwcProfiles_SplitSeason(um_data, ifs_data, misc_data, obs_data, month_flag, missing_files, um_out_dir, doy, obs_switch): #, lon, lat):
    ...
    if obs_switch == 'RADAR':
        ...
        plt.plot(np.nanmean(np.squeeze(ifs_data['model_lwc'][melt,:]),0)*1e3,np.nanmean(np.squeeze(ifs_data['height'][melt,:]),0), color = '#FFC107', linewidth = 3, label = 'ECMWF_IFS')
        ...
        plt.plot(np.nanmean(np.squeeze(ifs_data['model_lwc'][freeze,:]),0)*1e3,np.nanmean(np.squeeze(ifs_data['height'][freeze,:]),0), color = '#FFC107', linewidth = 3, label = 'ECMWF_IFS')
...
def plot_iwcProfiles_SplitSeason(um_data, ifs_data, misc_data, obs_data, month_flag, missing_files, um_out_dir, doy, obs_switch): #, lon, lat):
    ...
    plt.plot(np.nanmean(np.squeeze(ifs_data['model_snow_iwc_filtered'][melt,:]),0)*1e3,np.nanmean(np.squeeze(ifs_data['height'][melt,:]),0), color = '#FFC107', linewidth = 3, label = 'ECMWF_IFS')
    ...
    plt.plot(np.nanmean(np.squeeze(ifs_data['model_snow_iwc_filtered'][freeze,:]),0)*1e3,np.nanmean(np.squeeze(ifs_data['height'][freeze,:]),0), color = '#FFC107', linewidth = 3, label = 'ECMWF_IFS')
...
def plot_profiles_SplitSeason(um_data, ifs_data, misc_data, ra2t_data, obs_data, month_flag, missing_files, um_out_dir, doy, obs_switch): #, lon, lat):
    ...
    plt.plot(np.nanmean(np.squeeze(ifs_data['model_twc'][melt,:]),0)*1e3,np.nanmean(np.squeeze(ifs_data['height'][melt,:]),0), color = '#FFC107', linewidth = 3, label = 'ECMWF_IFS', zorder = 2)
    ...
    plt.plot(np.nanmean(np.squeeze(ifs_data['model_lwc'][melt,:]),0)*1e3,np.nanmean(np.squeeze(ifs_data['height'][melt,:]),0), color = '#FFC107', linewidth = 3, label = 'ECMWF_IFS', zorder = 2)
    ...
    plt.plot(np.nanmean(np.squeeze(ifs_data['model_snow_iwc_filtered'][melt,:]),0)*1e3,np.nanmean(np.squeeze(ifs_data['height'][melt,:]),0), color = '#FFC107', linewidth = 3, label = 'ECMWF_IFS', zorder = 2)
    ...
    plt.plot(np.nanmean(np.squeeze(ifs_data['model_twc'][freeze,:]),0)*1e3,np.nanmean(np.squeeze(ifs_data['height'][freeze,:]),0), color = '#FFC107', linewidth = 3, label = 'ECMWF_IFS', zorder = 2)
    ...
    plt.plot(np.nanmean(np.squeeze(ifs_data['model_lwc'][freeze,:]),0)*1e3,np.nanmean(np.squeeze(ifs_data['height'][freeze,:]),0), color = '#FFC107', linewidth = 3, label = 'ECMWF_IFS', zorder = 2)
    ax2.fill_betweenx(np.nanmean(np.squeeze(ifs_data['height'][freeze,:]),0),np.nanmean(np.squeeze(ifs_data['model_lwc'][freeze,:]),0)*1e3 - np.nanstd(np.squeeze(ifs_data['model_lwc'][freeze,:]),0)*1e3,
        np.nanmean(np.squeeze(ifs_data['model_lwc'][freeze,:]),0)*1e3 + np.nanstd(np.squeeze(ifs_data['model_lwc'][freeze,:]),0)*1e3, color = 'navajowhite', alpha = 0.35)
    ...
    plt.plot(np.nanmean(np.squeeze(ifs_data['model_snow_iwc_filtered'][freeze,:]),0)*1e3,np.nanmean(np.squeeze(ifs_data['height'][freeze,:]),0), color = '#FFC107', linewidth = 3, label = 'ECMWF_IFS', zorder = 2)
    ax2.fill_betweenx(np.nanmean(np.squeeze(ifs_data['height'][freeze,:]),0),np.nanmean(np.squeeze(ifs_data['model_snow_iwc_filtered'][freeze,:]),0)*1e3 - np.nanstd(np.squeeze(ifs_data['model_snow_iwc_filtered'][freeze,:]),0)*1e3,
        np.nanmean(np.squeeze(ifs_data['model_snow_iwc_filtered'][freeze,:]),0)*1e3 + np.nanstd(np.squeeze(ifs_data['model_snow_iwc_filtered'][freeze,:]),0)*1e3, color = 'navajowhite', alpha = 0.35)
...
def plot_line_TSa(data1, data2, data3, month_flag, missing_files, out_dir1, out_dir2, out_dir3, obs, doy, label1, label2, label3):
    ...
    # if ifs_flag == True:
    #     plt.plot(data3['time'], data3['sfc_pressure'].data/1e2, color = '#FFC107', label = label3)
    # else:
    #     plt.plot(data3['time'], data3['sfc_pressure'].data/1e2, color = '#FFC107',label = label3)
    ...
    if ifs_flag == True:
        plt.plot(data3['time'], data3['sfc_net_lw'].data + data3['sfc_net_sw'].data, color = '#FFC107', label = label3)
    else:
        plt.plot(data3['time'], data3['surface_net_LW_radiation'].data + data3['surface_net_SW_radiation'].data, color = '#FFC107', label = label3)
    plt.title('CRF [W/m2]')
    # plt.legend()
    if month_flag == 8: ax.set_xlim([13.0, 31.0])
    if month_flag == 9: ax.set_xlim([1.0, 15.0])
    ...
    if ifs_flag == True:
        ax1.plot(data3['time'], data3['sfc_temp_2m'].data, color = '#FFC107', label = '2m')
    else:
        ax1.plot(data3['time'], data3['temp_1.5m'].data, color = '#FFC107')#, label = '2m')
    ...
    if ifs_flag == True:
        plt.plot(data3['time'], data3['sfc_net_sw'].data, color = '#FFC107', label = label3)
    else:
        plt.plot(data3['time'], data3['surface_net_SW_radiation'].data, color = '#FFC107', label = label3)
    ...
    if ifs_flag == True:
        plt.plot(data3['time'], data3['sfc_net_lw'].data, color = '#FFC107')
    else:
        plt.plot(data3['time'], data3['surface_net_LW_radiation'].data, color = '#FFC107')
    ...
    if ifs_flag == True:
        plt.plot(data3['time'], data3['sfc_down_sens_heat_flx'].data * -1.0, color = '#FFC107')
    else:
        plt.plot(data3['time'], data3['sensible_heat_flux'].data, color = '#FFC107')# * -1.0)
    ...
    if ifs_flag == True:
        plt.plot(data3['time'], data3['sfc_down_lat_heat_flx'].data * -1.0, color = '#FFC107')
    else:
        plt.plot(data3['time'], data3['latent_heat_flux'].data, color = '#FFC107')# * -1.0)
...
def plot_scaledBLCv_thetaE(data1, data2, data3, data4, um_data, ifs_data, misc_data, ra2t_data, obs_data, month_flag, missing_files, out_dir1, out_dir2, out_dir3, obs, doy, label1, label2, label3):
    ...
    plt.plot(np.nanmean(ifsmean,0),data3['scaledZ'], 'd-', color = '#FFC107', linewidth = 2, label = label3)
    plt.legend()
    plt.show()
...
def plot_scaledBLCv_JVInv(data1, data2, data3, um_data, ifs_data, misc_data, obs_data, month_flag, missing_files, out_dir1, out_dir2, out_dir3, obs, doy, label1, label2, label3):
    ...
    plt.plot(np.nanmean(data3['scaledCv']['mean'][::6,:],0),data3['scaledZ'], color = '#FFC107', linewidth = 2, label = label3)
    plt.legend()
    plt.show()
...
def plot_scaledBL_thetaE(data1, data2, data3, data4, um_data, ifs_data, misc_data, ra2t_data, obs_data, month_flag, missing_files, out_dir1, out_dir2, out_dir3, obs, doy, label1, label2, label3, label4, var):
    ...
    if var == 'Cv':
        ...
        misc_data['model_Cv_filtered'][misc_data['model_Cv_filtered'] < 0.0] = np.nan
        ra2t_data['model_Cv_filtered'][ra2t_data['model_Cv_filtered'] < 0.0] = np.nan
    # elif var == 'lwc_adiabatic_inc_nolwp':
    #     obs_data['lwc_adiabatic_inc_nolwp'][obs_data['lwc_adiabatic_inc_nolwp'] == -999] = 0.0
    #     # obs_data['lwc'][obs_data['lwc'] == 0] = np.nan
    #     um_data['model_lwc'][um_data['model_lwc'] < 0.0] = 0.0
    #     ifs_data['model_lwc'][ifs_data['model_lwc'] < 0.0] = 0.0
    ...
    #     obs_data['lwc_adiabatic_inc_nolwp'] = obs_data['lwc_adiabatic_inc_nolwp'] * 1e3
    #     um_data['model_lwc'] = um_data['model_lwc'] * 1e3
    #     misc_data['model_lwc'] = misc_data['model_lwc'] * 1e3
    #     ra2t_data['model_lwc'] = ra2t_data['model_lwc'] * 1e3
    ...
    #     ra2t_data['model_iwc_filtered'] = ra2t_data['model_iwc_filtered'] * 1e3
    #     ifs_data['model_snow_iwc_filtered'] = ifs_data['model_snow_iwc_filtered'] * 1e3

    ### flag out large iwc/lwc values from IFS caused by missing dates
    ifs_data['model_snow_iwc_filtered'][ifs_data['model_snow_iwc_filtered'] >= 5.0e-3] = np.nan
    ifs_data['model_lwc'][ifs_data['model_lwc'] >= 0.4] = np.nan
    ...
    for i in range(0,np.size(data1['scaledTime'])):     ## loop over time
        ...
        ###-----------------------------------------------------------------------------------------
        ### create array of height points under the identified inversion
        ###         only include heights above 155m
        um_Zindex = np.where(um_data['height'][i,:] < 155.0)
        ifs_Zindex = np.where(ifs_data['height'][i,:] < 155.0)
        if data1['inversions']['invbase_kIndex'][i] >= 0.0:
            hgts1 = um_data['height'][i,:int(data1['inversions']['invbase_kIndex'][i])]
        else:
            continue
    ...
    color = '#FFC107', markeredgecolor = 'saddlebrown', linewidth = 2, label = label3)
    ...
    if var == 'Cv':
        plt.xlim([0,1])
        plt.xlabel('C$_{V}$')
    if var == 'lwc_adiabatic_inc_nolwp':
        plt.xlim([0,0.2])
        plt.xlabel('LWC [g m$^{-2}$]')
    if var == 'iwc':
        plt.xlim([0,0.02])
        plt.xlabel('IWC [g m$^{-2}$]')
    plt.ylim([0,1])
...
def period_Selection(um_data, ifs_data, misc_data, ra2t_data, obs_data, month_flag, missing_files, um_out_dir, doy, obs_switch, obs, data1, data2, data3, data4, nanind, lwcind, iwcind):
    ...
    plt.plot(np.nanmean(fraction3p3,0), np.nanmean(np.squeeze(ifs_data['height'][p3,:]),0), color = '#FFC107', linewidth = 2, label = 'ECMWF_IFS', zorder = 4)
    ...
    plt.plot(np.nanmean(np.squeeze(obs_data['lwc_adiabatic_inc_nolwp'][p3,lwc_binlimit:]),0)*1e3 - np.nanstd(np.squeeze(obs_data['lwc_adiabatic_inc_nolwp'][p3,lwc_binlimit:]),0)*1e3,
        np.nanmean(np.squeeze(obs_data['height'][p3,lwc_binlimit:]),0),
        '--', color = 'k', linewidth = 0.5)
    plt.plot(np.nanmean(np.squeeze(obs_data['lwc_adiabatic_inc_nolwp'][p3,lwc_binlimit:]),0)*1e3 + np.nanstd(np.squeeze(obs_data['lwc_adiabatic_inc_nolwp'][p3,lwc_binlimit:]),0)*1e3,
        np.nanmean(np.squeeze(obs_data['height'][p3,lwc_binlimit:]),0),
        '--', color = 'k', linewidth = 0.5)

    ax2.fill_betweenx(np.nanmean(np.squeeze(um_data['height'][p3,lwc_binlimit:]),0),
        np.nanmean(np.squeeze(um_data['model_lwc'][p3,lwc_binlimit:]),0)*1e3 - np.nanstd(np.squeeze(um_data['model_lwc'][p3,lwc_binlimit:]),0)*1e3,
        np.nanmean(np.squeeze(um_data['model_lwc'][p3,lwc_binlimit:]),0)*1e3 + np.nanstd(np.squeeze(um_data['model_lwc'][p3,lwc_binlimit:]),0)*1e3,
        color = 'blue', alpha = 0.05)
    plt.plot(np.nanmean(np.squeeze(um_data['model_lwc'][p3,lwc_binlimit:]),0)*1e3 - np.nanstd(np.squeeze(um_data['model_lwc'][p3,lwc_binlimit:]),0)*1e3,
        np.nanmean(np.squeeze(um_data['height'][p3,lwc_binlimit:]),0),
        '--', color = 'darkblue', linewidth = 0.5)
    plt.plot(np.nanmean(np.squeeze(um_data['model_lwc'][p3,lwc_binlimit:]),0)*1e3 + np.nanstd(np.squeeze(um_data['model_lwc'][p3,lwc_binlimit:]),0)*1e3,
        np.nanmean(np.squeeze(um_data['height'][p3,lwc_binlimit:]),0),
        '--', color = 'darkblue', linewidth = 0.5)

    ax2.fill_betweenx(np.nanmean(np.squeeze(ra2t_data['height'][p3,lwc_binlimit:]),0),
        np.nanmean(np.squeeze(ra2t_data['model_lwc'][p3,lwc_binlimit:]),0)*1e3 - np.nanstd(np.squeeze(ra2t_data['model_lwc'][p3,lwc_binlimit:]),0)*1e3,
        np.nanmean(np.squeeze(ra2t_data['model_lwc'][p3,lwc_binlimit:]),0)*1e3 + np.nanstd(np.squeeze(ra2t_data['model_lwc'][p3,lwc_binlimit:]),0)*1e3,
        color = 'lightblue', alpha = 0.15)
    plt.plot(np.nanmean(np.squeeze(ra2t_data['model_lwc'][p3,lwc_binlimit:]),0)*1e3 - np.nanstd(np.squeeze(ra2t_data['model_lwc'][p3,lwc_binlimit:]),0)*1e3,
        np.nanmean(np.squeeze(ra2t_data['height'][p3,lwc_binlimit:]),0),
        '--', color = 'steelblue', linewidth = 0.5)
    plt.plot(np.nanmean(np.squeeze(ra2t_data['model_lwc'][p3,lwc_binlimit:]),0)*1e3 + np.nanstd(np.squeeze(ra2t_data['model_lwc'][p3,lwc_binlimit:]),0)*1e3,
        np.nanmean(np.squeeze(ra2t_data['height'][p3,lwc_binlimit:]),0),
        '--', color = 'steelblue', linewidth = 0.5)

    ax2.fill_betweenx(np.nanmean(np.squeeze(misc_data['height'][p3,lwc_binlimit:]),0),
        np.nanmean(np.squeeze(misc_data['model_lwc'][p3,lwc_binlimit:]),0)*1e3 - np.nanstd(np.squeeze(misc_data['model_lwc'][p3,lwc_binlimit:]),0)*1e3,
        np.nanmean(np.squeeze(misc_data['model_lwc'][p3,lwc_binlimit:]),0)*1e3 + np.nanstd(np.squeeze(misc_data['model_lwc'][p3,lwc_binlimit:]),0)*1e3,
        color = 'mediumaquamarine', alpha = 0.15)
    plt.plot(np.nanmean(np.squeeze(misc_data['model_lwc'][p3,lwc_binlimit:]),0)*1e3 - np.nanstd(np.squeeze(misc_data['model_lwc'][p3,lwc_binlimit:]),0)*1e3,
        np.nanmean(np.squeeze(misc_data['height'][p3,lwc_binlimit:]),0),
        '--', color = 'mediumseagreen', linewidth = 0.5)
    plt.plot(np.nanmean(np.squeeze(misc_data['model_lwc'][p3,lwc_binlimit:]),0)*1e3 + np.nanstd(np.squeeze(misc_data['model_lwc'][p3,lwc_binlimit:]),0)*1e3,
        np.nanmean(np.squeeze(misc_data['height'][p3,lwc_binlimit:]),0),
        '--', color = 'mediumseagreen', linewidth = 0.5)

    ax2.fill_betweenx(np.nanmean(np.squeeze(ifs_data['height'][p3,lwc_binlimit:]),0),
        np.nanmean(np.squeeze(ifs_data['model_lwc'][p3,lwc_binlimit:]),0)*1e3 - np.nanstd(np.squeeze(ifs_data['model_lwc'][p3,lwc_binlimit:]),0)*1e3,
        np.nanmean(np.squeeze(ifs_data['model_lwc'][p3,lwc_binlimit:]),0)*1e3 + np.nanstd(np.squeeze(ifs_data['model_lwc'][p3,lwc_binlimit:]),0)*1e3,
        color = 'navajowhite', alpha = 0.35)
    plt.plot(np.nanmean(np.squeeze(ifs_data['model_lwc'][p3,lwc_binlimit:]),0)*1e3 - np.nanstd(np.squeeze(ifs_data['model_lwc'][p3,lwc_binlimit:]),0)*1e3,
        np.nanmean(np.squeeze(ifs_data['height'][p3,lwc_binlimit:]),0),
        '--', color = '#FFC107', linewidth = 0.5)
    plt.plot(np.nanmean(np.squeeze(ifs_data['model_lwc'][p3,lwc_binlimit:]),0)*1e3 + np.nanstd(np.squeeze(ifs_data['model_lwc'][p3,lwc_binlimit:]),0)*1e3,
        np.nanmean(np.squeeze(ifs_data['height'][p3,lwc_binlimit:]),0),
        '--', color = '#FFC107', linewidth = 0.5)

    plt.plot(np.nanmean(np.squeeze(obs_data['lwc'][p3,lwc_binlimit:]),0)*1e3,np.nanmean(np.squeeze(obs_data['height'][p3,lwc_binlimit:]),0),
        'k', linewidth = 3, label = 'Obs_' + obs_switch + 'grid-adb', zorder = 3)
    plt.plot(np.nanmean(np.squeeze(ifs_data['model_lwc'][p3,lwc_binlimit:]),0)*1e3,np.nanmean(np.squeeze(ifs_data['height'][p3,lwc_binlimit:]),0),
        color = '#FFC107', linewidth = 2, label = 'ECMWF_IFS', zorder = 2)
    plt.plot(np.nanmean(np.squeeze(misc_data['model_lwc'][p3,lwc_binlimit:]),0)*1e3,np.nanmean(np.squeeze(misc_data['height'][p3,lwc_binlimit:]),0),
        color = 'mediumseagreen', linewidth = 2, label = 'UM_CASIM-100', zorder = 1)
    plt.plot(np.nanmean(np.squeeze(ra2t_data['model_lwc'][p3,lwc_binlimit:]),0)*1e3,np.nanmean(np.squeeze(ra2t_data['height'][p3,lwc_binlimit:]),0),
        color = 'steelblue', linewidth = 2, label = 'UM_RA2T', zorder = 1)
    plt.plot(np.nanmean(np.squeeze(um_data['model_lwc'][p3,lwc_binlimit:]),0)*1e3,np.nanmean(np.squeeze(um_data['height'][p3,lwc_binlimit:]),0),
    ...
    ax2.fill_betweenx(np.nanmean(np.squeeze(obs_data['height'][p3,:]),0),
        np.nanmean(np.squeeze(obs_data['iwc'][p3,:]),0)*1e3 - np.nanstd(np.squeeze(obs_data['iwc'][p3,:]),0)*1e3,
        np.nanmean(np.squeeze(obs_data['iwc'][p3,:]),0)*1e3 + np.nanstd(np.squeeze(obs_data['iwc'][p3,:]),0)*1e3,
        color = 'lightgrey', alpha = 0.5)
    plt.plot(np.nanmean(np.squeeze(obs_data['iwc'][p3,:]),0)*1e3 - np.nanstd(np.squeeze(obs_data['iwc'][p3,:]),0)*1e3,
        np.nanmean(np.squeeze(obs_data['height'][p3,:]),0),
        '--', color = 'k', linewidth = 0.5)
    plt.plot(np.nanmean(np.squeeze(obs_data['iwc'][p3,:]),0)*1e3 + np.nanstd(np.squeeze(obs_data['iwc'][p3,:]),0)*1e3,
        np.nanmean(np.squeeze(obs_data['height'][p3,:]),0),
    ...
    plt.plot(np.nanmean(np.squeeze(um_data['model_iwc_filtered'][p3,:]),0)*1e3,
        np.nanmean(np.squeeze(um_data['height'][p3,:]),0),
        color = 'darkblue', linewidth = 2, label = 'UM_RA2M', zorder = 2)
    ax2.fill_betweenx(np.nanmean(np.squeeze(um_data['height'][p3,:]),0),
        np.nanmean(np.squeeze(um_data['model_iwc_filtered'][p3,:]),0)*1e3 - np.nanstd(np.squeeze(um_data['model_iwc_filtered'][p3,:]),0)*1e3,
        np.nanmean(np.squeeze(um_data['model_iwc_filtered'][p3,:]),0)*1e3 + np.nanstd(np.squeeze(um_data['model_iwc_filtered'][p3,:]),0)*1e3,
        color = 'blue', alpha = 0.05)
    plt.plot(np.nanmean(np.squeeze(um_data['model_iwc_filtered'][p3,:]),0)*1e3 - np.nanstd(np.squeeze(um_data['model_iwc_filtered'][p3,:]),0)*1e3,
    ...
    plt.plot(np.nanmean(np.squeeze(ra2t_data['model_iwc_filtered'][p3,:]),0)*1e3,
        np.nanmean(np.squeeze(ra2t_data['height'][p3,:]),0),
        color = 'steelblue', linewidth = 2, label = 'UM_RA2T', zorder = 1)
    ax2.fill_betweenx(np.nanmean(np.squeeze(ra2t_data['height'][p3,:]),0),
        np.nanmean(np.squeeze(ra2t_data['model_iwc_filtered'][p3,:]),0)*1e3 - np.nanstd(np.squeeze(ra2t_data['model_iwc_filtered'][p3,:]),0)*1e3,
        np.nanmean(np.squeeze(ra2t_data['model_iwc_filtered'][p3,:]),0)*1e3 + np.nanstd(np.squeeze(ra2t_data['model_iwc_filtered'][p3,:]),0)*1e3,
        color = 'lightblue', alpha = 0.15)
    plt.plot(np.nanmean(np.squeeze(ra2t_data['model_iwc_filtered'][p3,:]),0)*1e3 - np.nanstd(np.squeeze(ra2t_data['model_iwc_filtered'][p3,:]),0)*1e3,
    ...
    plt.plot(np.nanmean(np.squeeze(misc_data['model_iwc_filtered'][p3,:]),0)*1e3,
        np.nanmean(np.squeeze(misc_data['height'][p3,:]),0),
        color = 'mediumseagreen', linewidth = 2, label = 'UM_CASIM-100', zorder = 1)
    ax2.fill_betweenx(np.nanmean(np.squeeze(misc_data['height'][p3,:]),0),
        np.nanmean(np.squeeze(misc_data['model_iwc_filtered'][p3,:]),0)*1e3 - np.nanstd(np.squeeze(misc_data['model_iwc_filtered'][p3,:]),0)*1e3,
        np.nanmean(np.squeeze(misc_data['model_iwc_filtered'][p3,:]),0)*1e3 + np.nanstd(np.squeeze(misc_data['model_iwc_filtered'][p3,:]),0)*1e3,
        color = 'mediumaquamarine', alpha = 0.15)
    plt.plot(np.nanmean(np.squeeze(misc_data['model_iwc_filtered'][p3,:]),0)*1e3 - np.nanstd(np.squeeze(misc_data['model_iwc_filtered'][p3,:]),0)*1e3,
    ...
    plt.plot(np.nanmean(np.squeeze(ifs_data['model_snow_iwc_filtered'][p3,:]),0)*1e3,
        np.nanmean(np.squeeze(ifs_data['height'][p3,:]),0),
        color = '#FFC107', linewidth = 2, label = 'ECMWF_IFS', zorder = 2)
    ax2.fill_betweenx(np.nanmean(np.squeeze(ifs_data['height'][p3,:]),0),
        np.nanmean(np.squeeze(ifs_data['model_snow_iwc_filtered'][p3,:]),0)*1e3 - np.nanstd(np.squeeze(ifs_data['model_snow_iwc_filtered'][p3,:]),0)*1e3,
        np.nanmean(np.squeeze(ifs_data['model_snow_iwc_filtered'][p3,:]),0)*1e3 + np.nanstd(np.squeeze(ifs_data['model_snow_iwc_filtered'][p3,:]),0)*1e3,
        color = 'navajowhite', alpha = 0.35)
    plt.plot(np.nanmean(np.squeeze(ifs_data['model_snow_iwc_filtered'][p3,:]),0)*1e3 - np.nanstd(np.squeeze(ifs_data['model_snow_iwc_filtered'][p3,:]),0)*1e3,
        np.nanmean(np.squeeze(ifs_data['height'][p3,:]),0),
        '--', color = '#FFC107', linewidth = 0.5)
    plt.plot(np.nanmean(np.squeeze(ifs_data['model_snow_iwc_filtered'][p3,:]),0)*1e3 + np.nanstd(np.squeeze(ifs_data['model_snow_iwc_filtered'][p3,:]),0)*1e3,
        np.nanmean(np.squeeze(ifs_data['height'][p3,:]),0),
    ...
    ax2.fill_betweenx(np.nanmean(np.squeeze(obs_data['height'][p6,:]),0),
        ...
        np.nanmean(fraction0p6,0) + np.nanstd(fraction0p6,0),
        color = 'lightgrey', alpha = 0.5)
    plt.plot(np.nanmean(fraction0p6,0) - np.nanstd(fraction0p6,0),
    ...
    ax2.fill_betweenx(np.nanmean(np.squeeze(um_data['height'][p6,:]),0),
        ...
        np.nanmean(fraction1p6,0) + np.nanstd(fraction1p6,0),
        color = 'blue', alpha = 0.05)
    plt.plot(np.nanmean(fraction1p6,0) - np.nanstd(fraction1p6,0),
        np.nanmean(um_data['height'],0),
        '--', color = 'darkblue', linewidth = 0.5)
    plt.plot(np.nanmean(fraction1p6,0) + np.nanstd(fraction1p6,0),
        np.nanmean(um_data['height'],0),
    ...
    ax2.fill_betweenx(np.nanmean(np.squeeze(ra2t_data['height'][p6,:]),0),
        ...
        np.nanmean(fraction4p6,0) + np.nanstd(fraction4p6,0),
        color = 'lightblue', alpha = 0.15)
    plt.plot(np.nanmean(fraction4p6,0) - np.nanstd(fraction4p6,0),
        np.nanmean(ra2t_data['height'],0),
        '--', color = 'steelblue', linewidth = 0.5)
    plt.plot(np.nanmean(fraction4p6,0) + np.nanstd(fraction4p6,0),
        np.nanmean(ra2t_data['height'],0),
    ...
    ax2.fill_betweenx(np.nanmean(np.squeeze(misc_data['height'][p6,:]),0),
        ...
        np.nanmean(fraction2p6,0) + np.nanstd(fraction2p6,0),
        color = 'mediumaquamarine', alpha = 0.15)
    plt.plot(np.nanmean(fraction2p6,0) - np.nanstd(fraction2p6,0),
        np.nanmean(misc_data['height'],0),
        '--', color = 'mediumseagreen', linewidth = 0.5)
    plt.plot(np.nanmean(fraction2p6,0) + np.nanstd(fraction2p6,0),
        np.nanmean(misc_data['height'],0),
    ...
    ax2.fill_betweenx(np.nanmean(np.squeeze(obs_data['height'][p6,lwc_binlimit:]),0),
        ...
        np.nanmean(np.squeeze(obs_data['lwc_adiabatic_inc_nolwp'][p6,lwc_binlimit:]),0)*1e3 + np.nanstd(np.squeeze(obs_data['lwc_adiabatic_inc_nolwp'][p6,lwc_binlimit:]),0)*1e3,
        color = 'lightgrey', alpha = 0.5)
    plt.plot(np.nanmean(np.squeeze(obs_data['lwc_adiabatic_inc_nolwp'][p6,lwc_binlimit:]),0)*1e3 - np.nanstd(np.squeeze(obs_data['lwc_adiabatic_inc_nolwp'][p6,lwc_binlimit:]),0)*1e3,
        np.nanmean(np.squeeze(obs_data['height'][p6,lwc_binlimit:]),0),
        '--', color = 'k', linewidth = 0.5)
    plt.plot(np.nanmean(np.squeeze(obs_data['lwc_adiabatic_inc_nolwp'][p6,lwc_binlimit:]),0)*1e3 + np.nanstd(np.squeeze(obs_data['lwc_adiabatic_inc_nolwp'][p6,lwc_binlimit:]),0)*1e3,
        np.nanmean(np.squeeze(obs_data['height'][p6,lwc_binlimit:]),0),
    ...
    ax2.fill_betweenx(np.nanmean(np.squeeze(um_data['height'][p6,lwc_binlimit:]),0),
        ...
        np.nanmean(np.squeeze(um_data['model_lwc'][p6,lwc_binlimit:]),0)*1e3 + np.nanstd(np.squeeze(um_data['model_lwc'][p6,lwc_binlimit:]),0)*1e3,
        color = 'blue', alpha = 0.05)
    plt.plot(np.nanmean(np.squeeze(um_data['model_lwc'][p6,lwc_binlimit:]),0)*1e3 - np.nanstd(np.squeeze(um_data['model_lwc'][p6,lwc_binlimit:]),0)*1e3,
        np.nanmean(np.squeeze(um_data['height'][p6,lwc_binlimit:]),0),
        '--', color = 'darkblue', linewidth = 0.5)
    plt.plot(np.nanmean(np.squeeze(um_data['model_lwc'][p6,lwc_binlimit:]),0)*1e3 + np.nanstd(np.squeeze(um_data['model_lwc'][p6,lwc_binlimit:]),0)*1e3,
        np.nanmean(np.squeeze(um_data['height'][p6,lwc_binlimit:]),0),
    ...
    ax2.fill_betweenx(np.nanmean(np.squeeze(ra2t_data['height'][p6,lwc_binlimit:]),0),
        ...
        np.nanmean(np.squeeze(ra2t_data['model_lwc'][p6,lwc_binlimit:]),0)*1e3 + np.nanstd(np.squeeze(ra2t_data['model_lwc'][p6,lwc_binlimit:]),0)*1e3,
        color = 'lightblue', alpha = 0.15)
    plt.plot(np.nanmean(np.squeeze(ra2t_data['model_lwc'][p6,lwc_binlimit:]),0)*1e3 - np.nanstd(np.squeeze(ra2t_data['model_lwc'][p6,lwc_binlimit:]),0)*1e3,
        np.nanmean(np.squeeze(ra2t_data['height'][p6,lwc_binlimit:]),0),
        '--', color = 'steelblue', linewidth = 0.5)
    plt.plot(np.nanmean(np.squeeze(ra2t_data['model_lwc'][p6,lwc_binlimit:]),0)*1e3 + np.nanstd(np.squeeze(ra2t_data['model_lwc'][p6,lwc_binlimit:]),0)*1e3,
        np.nanmean(np.squeeze(ra2t_data['height'][p6,lwc_binlimit:]),0),
    ...
    ax2.fill_betweenx(np.nanmean(np.squeeze(misc_data['height'][p6,lwc_binlimit:]),0),
        ...
        np.nanmean(np.squeeze(misc_data['model_lwc'][p6,lwc_binlimit:]),0)*1e3 + np.nanstd(np.squeeze(misc_data['model_lwc'][p6,lwc_binlimit:]),0)*1e3,
        color = 'mediumaquamarine', alpha = 0.15)
    plt.plot(np.nanmean(np.squeeze(misc_data['model_lwc'][p6,lwc_binlimit:]),0)*1e3 - np.nanstd(np.squeeze(misc_data['model_lwc'][p6,lwc_binlimit:]),0)*1e3,
        np.nanmean(np.squeeze(misc_data['height'][p6,lwc_binlimit:]),0),
        '--', color = 'mediumseagreen', linewidth = 0.5)
    plt.plot(np.nanmean(np.squeeze(misc_data['model_lwc'][p6,lwc_binlimit:]),0)*1e3 + np.nanstd(np.squeeze(misc_data['model_lwc'][p6,lwc_binlimit:]),0)*1e3,
        np.nanmean(np.squeeze(misc_data['height'][p6,lwc_binlimit:]),0),
    ...
    ax2.fill_betweenx(np.nanmean(np.squeeze(obs_data['height'][p6,:]),0),
        ...
        np.nanmean(np.squeeze(obs_data['iwc'][p6,:]),0)*1e3 + np.nanstd(np.squeeze(obs_data['iwc'][p6,:]),0)*1e3,
        color = 'lightgrey', alpha = 0.5)
    plt.plot(np.nanmean(np.squeeze(obs_data['iwc'][p6,:]),0)*1e3 - np.nanstd(np.squeeze(obs_data['iwc'][p6,:]),0)*1e3,
        np.nanmean(np.squeeze(obs_data['height'][p6,:]),0),
        '--', color = 'k', linewidth = 0.5)
    plt.plot(np.nanmean(np.squeeze(obs_data['iwc'][p6,:]),0)*1e3 + np.nanstd(np.squeeze(obs_data['iwc'][p6,:]),0)*1e3,
        np.nanmean(np.squeeze(obs_data['height'][p6,:]),0),
    ...
    ax2.fill_betweenx(np.nanmean(np.squeeze(um_data['height'][p6,:]),0),
        ...
        np.nanmean(np.squeeze(um_data['model_iwc_filtered'][p6,:]),0)*1e3 + np.nanstd(np.squeeze(um_data['model_iwc_filtered'][p6,:]),0)*1e3,
        color = 'blue', alpha = 0.05)
    plt.plot(np.nanmean(np.squeeze(um_data['model_iwc_filtered'][p6,:]),0)*1e3 - np.nanstd(np.squeeze(um_data['model_iwc_filtered'][p6,:]),0)*1e3,
        np.nanmean(um_data['height'],0),
        '--', color = 'darkblue', linewidth = 0.5)
    plt.plot(np.nanmean(np.squeeze(um_data['model_iwc_filtered'][p6,:]),0)*1e3 + np.nanstd(np.squeeze(um_data['model_iwc_filtered'][p6,:]),0)*1e3,
        np.nanmean(um_data['height'],0),
    ...
    ax2.fill_betweenx(np.nanmean(np.squeeze(ra2t_data['height'][p6,:]),0),
        ...
        np.nanmean(np.squeeze(ra2t_data['model_iwc_filtered'][p6,:]),0)*1e3 + np.nanstd(np.squeeze(ra2t_data['model_iwc_filtered'][p6,:]),0)*1e3,
        color = 'lightblue', alpha = 0.15)
    plt.plot(np.nanmean(np.squeeze(ra2t_data['model_iwc_filtered'][p6,:]),0)*1e3 - np.nanstd(np.squeeze(ra2t_data['model_iwc_filtered'][p6,:]),0)*1e3,
        np.nanmean(ra2t_data['height'],0),
        '--', color = 'steelblue', linewidth = 0.5)
    plt.plot(np.nanmean(np.squeeze(ra2t_data['model_iwc_filtered'][p6,:]),0)*1e3 + np.nanstd(np.squeeze(ra2t_data['model_iwc_filtered'][p6,:]),0)*1e3,
        np.nanmean(ra2t_data['height'],0),
    ...
    ax2.fill_betweenx(np.nanmean(np.squeeze(misc_data['height'][p6,:]),0),
        ...
        np.nanmean(np.squeeze(misc_data['model_iwc_filtered'][p6,:]),0)*1e3 + np.nanstd(np.squeeze(misc_data['model_iwc_filtered'][p6,:]),0)*1e3,
        color = 'mediumaquamarine', alpha = 0.15)
    plt.plot(np.nanmean(np.squeeze(misc_data['model_iwc_filtered'][p6,:]),0)*1e3 - np.nanstd(np.squeeze(misc_data['model_iwc_filtered'][p6,:]),0)*1e3,
        np.nanmean(misc_data['height'],0),
        '--', color = 'mediumseagreen', linewidth = 0.5)
    plt.plot(np.nanmean(np.squeeze(misc_data['model_iwc_filtered'][p6,:]),0)*1e3 + np.nanstd(np.squeeze(misc_data['model_iwc_filtered'][p6,:]),0)*1e3,
        np.nanmean(misc_data['height'],0),
    ...
    plt.plot(np.nanmean(fraction0p4,0),np.nanmean(np.squeeze(obs_data['height'][p4,:]),0), 'k', linewidth = 3, label = 'Obs_' + obs_switch + 'grid', zorder = 5)
    plt.plot(np.nanmean(fraction3p4,0), np.nanmean(np.squeeze(ifs_data['height'][p4,:]),0), color = '#FFC107', linewidth = 2, label = 'ECMWF_IFS', zorder = 4)
    ...
    plt.plot(np.nanmean(fraction4p4,0),np.nanmean(np.squeeze(ra2t_data['height'][p4,:]),0), color = 'steelblue', linewidth = 2, label = 'UM_RA2T', zorder = 2)
    plt.plot(np.nanmean(fraction1p4,0),np.nanmean(np.squeeze(um_data['height'][p4,:]),0), color = 'darkblue', linewidth = 2, label = 'UM_RA2M', zorder = 3)
    ...
    ax2.fill_betweenx(np.nanmean(np.squeeze(um_data['height'][p4,lwc_binlimit:]),0),
        ...
        np.nanmean(np.squeeze(um_data['model_lwc'][p4,lwc_binlimit:]),0)*1e3 + np.nanstd(np.squeeze(um_data['model_lwc'][p4,lwc_binlimit:]),0)*1e3,
        color = 'blue', alpha = 0.05)
    plt.plot(np.nanmean(np.squeeze(um_data['model_lwc'][p4,lwc_binlimit:]),0)*1e3 - np.nanstd(np.squeeze(um_data['model_lwc'][p4,lwc_binlimit:]),0)*1e3,
        np.nanmean(np.squeeze(um_data['height'][p4,lwc_binlimit:]),0),
        '--', color = 'darkblue', linewidth = 0.5)
    plt.plot(np.nanmean(np.squeeze(um_data['model_lwc'][p4,lwc_binlimit:]),0)*1e3 + np.nanstd(np.squeeze(um_data['model_lwc'][p4,lwc_binlimit:]),0)*1e3,
        np.nanmean(np.squeeze(um_data['height'][p4,lwc_binlimit:]),0),
        '--', color = 'darkblue', linewidth = 0.5)

    ax2.fill_betweenx(np.nanmean(np.squeeze(ra2t_data['height'][p4,lwc_binlimit:]),0),
        np.nanmean(np.squeeze(ra2t_data['model_lwc'][p4,lwc_binlimit:]),0)*1e3 - np.nanstd(np.squeeze(ra2t_data['model_lwc'][p4,lwc_binlimit:]),0)*1e3,
        np.nanmean(np.squeeze(ra2t_data['model_lwc'][p4,lwc_binlimit:]),0)*1e3 + np.nanstd(np.squeeze(ra2t_data['model_lwc'][p4,lwc_binlimit:]),0)*1e3,
        color = 'lightblue', alpha = 0.15)
    plt.plot(np.nanmean(np.squeeze(ra2t_data['model_lwc'][p4,lwc_binlimit:]),0)*1e3 - np.nanstd(np.squeeze(ra2t_data['model_lwc'][p4,lwc_binlimit:]),0)*1e3,
        np.nanmean(np.squeeze(ra2t_data['height'][p4,lwc_binlimit:]),0),
        '--', color = 'steelblue', linewidth = 0.5)
    plt.plot(np.nanmean(np.squeeze(ra2t_data['model_lwc'][p4,lwc_binlimit:]),0)*1e3 + np.nanstd(np.squeeze(ra2t_data['model_lwc'][p4,lwc_binlimit:]),0)*1e3,
        np.nanmean(np.squeeze(ra2t_data['height'][p4,lwc_binlimit:]),0),
        '--', color = 'steelblue', linewidth = 0.5)

    ax2.fill_betweenx(np.nanmean(np.squeeze(misc_data['height'][p4,lwc_binlimit:]),0),
        np.nanmean(np.squeeze(misc_data['model_lwc'][p4,lwc_binlimit:]),0)*1e3 - np.nanstd(np.squeeze(misc_data['model_lwc'][p4,lwc_binlimit:]),0)*1e3,
        np.nanmean(np.squeeze(misc_data['model_lwc'][p4,lwc_binlimit:]),0)*1e3 + np.nanstd(np.squeeze(misc_data['model_lwc'][p4,lwc_binlimit:]),0)*1e3,
        color = 'mediumaquamarine', alpha = 0.15)
    plt.plot(np.nanmean(np.squeeze(misc_data['model_lwc'][p4,lwc_binlimit:]),0)*1e3 - np.nanstd(np.squeeze(misc_data['model_lwc'][p4,lwc_binlimit:]),0)*1e3,
        np.nanmean(np.squeeze(misc_data['height'][p4,lwc_binlimit:]),0),
        '--', color = 'mediumseagreen', linewidth = 0.5)
    plt.plot(np.nanmean(np.squeeze(misc_data['model_lwc'][p4,lwc_binlimit:]),0)*1e3 + np.nanstd(np.squeeze(misc_data['model_lwc'][p4,lwc_binlimit:]),0)*1e3,
        np.nanmean(np.squeeze(misc_data['height'][p4,lwc_binlimit:]),0),
        '--', color = 'mediumseagreen', linewidth = 0.5)

    ax2.fill_betweenx(np.nanmean(np.squeeze(ifs_data['height'][p4,lwc_binlimit:]),0),
        np.nanmean(np.squeeze(ifs_data['model_lwc'][p4,lwc_binlimit:]),0)*1e3 - np.nanstd(np.squeeze(ifs_data['model_lwc'][p4,lwc_binlimit:]),0)*1e3,
        np.nanmean(np.squeeze(ifs_data['model_lwc'][p4,lwc_binlimit:]),0)*1e3 + np.nanstd(np.squeeze(ifs_data['model_lwc'][p4,lwc_binlimit:]),0)*1e3,
        color = 'navajowhite', alpha = 0.35)
    plt.plot(np.nanmean(np.squeeze(ifs_data['model_lwc'][p4,lwc_binlimit:]),0)*1e3 - np.nanstd(np.squeeze(ifs_data['model_lwc'][p4,lwc_binlimit:]),0)*1e3,
        np.nanmean(np.squeeze(ifs_data['height'][p4,lwc_binlimit:]),0),
        '--', color = '#FFC107', linewidth = 0.5)
    plt.plot(np.nanmean(np.squeeze(ifs_data['model_lwc'][p4,lwc_binlimit:]),0)*1e3 + np.nanstd(np.squeeze(ifs_data['model_lwc'][p4,lwc_binlimit:]),0)*1e3,
        np.nanmean(np.squeeze(ifs_data['height'][p4,lwc_binlimit:]),0),
        '--', color = '#FFC107', linewidth = 0.5)

    plt.plot(np.nanmean(np.squeeze(obs_data['lwc'][p4,lwc_binlimit:]),0)*1e3,np.nanmean(np.squeeze(obs_data['height'][p4,lwc_binlimit:]),0),
        'k', linewidth = 3, label = 'Obs_' + obs_switch + 'grid-adb', zorder = 3)
    plt.plot(np.nanmean(np.squeeze(ifs_data['model_lwc'][p4,lwc_binlimit:]),0)*1e3,np.nanmean(np.squeeze(ifs_data['height'][p4,lwc_binlimit:]),0),
        color = '#FFC107', linewidth = 2, label = 'ECMWF_IFS', zorder = 2)
    plt.plot(np.nanmean(np.squeeze(misc_data['model_lwc'][p4,lwc_binlimit:]),0)*1e3,np.nanmean(np.squeeze(misc_data['height'][p4,lwc_binlimit:]),0),
        color = 'mediumseagreen', linewidth = 2, label = 'UM_CASIM-100', zorder = 1)
    plt.plot(np.nanmean(np.squeeze(ra2t_data['model_lwc'][p4,lwc_binlimit:]),0)*1e3,np.nanmean(np.squeeze(ra2t_data['height'][p4,lwc_binlimit:]),0),
        color = 'steelblue', linewidth = 2, label = 'UM_RA2T', zorder = 1)
    plt.plot(np.nanmean(np.squeeze(um_data['model_lwc'][p4,lwc_binlimit:]),0)*1e3,np.nanmean(np.squeeze(um_data['height'][p4,lwc_binlimit:]),0),
        color = 'darkblue', linewidth = 2, label = 'UM_RA2M', zorder = 2)
    ...
    ax2.fill_betweenx(np.nanmean(np.squeeze(obs_data['height'][p4,:]),0),
        ...
        np.nanmean(np.squeeze(obs_data['iwc'][p4,:]),0)*1e3 + np.nanstd(np.squeeze(obs_data['iwc'][p4,:]),0)*1e3,
        color = 'lightgrey', alpha = 0.5)
    plt.plot(np.nanmean(np.squeeze(obs_data['iwc'][p4,:]),0)*1e3 - np.nanstd(np.squeeze(obs_data['iwc'][p4,:]),0)*1e3,
        np.nanmean(np.squeeze(obs_data['height'][p4,:]),0),
        '--', color = 'k', linewidth = 0.5)
    plt.plot(np.nanmean(np.squeeze(obs_data['iwc'][p4,:]),0)*1e3 + np.nanstd(np.squeeze(obs_data['iwc'][p4,:]),0)*1e3,
        np.nanmean(np.squeeze(obs_data['height'][p4,:]),0),
    ...
    ax2.fill_betweenx(np.nanmean(np.squeeze(um_data['height'][p4,:]),0),
        ...
        np.nanmean(np.squeeze(um_data['model_iwc_filtered'][p4,:]),0)*1e3 + np.nanstd(np.squeeze(um_data['model_iwc_filtered'][p4,:]),0)*1e3,
        color = 'blue', alpha = 0.05)
    plt.plot(np.nanmean(np.squeeze(um_data['model_iwc_filtered'][p4,:]),0)*1e3 - np.nanstd(np.squeeze(um_data['model_iwc_filtered'][p4,:]),0)*1e3,
        np.nanmean(um_data['height'],0),
        '--', color = 'darkblue', linewidth = 0.5)
    plt.plot(np.nanmean(np.squeeze(um_data['model_iwc_filtered'][p4,:]),0)*1e3 + np.nanstd(np.squeeze(um_data['model_iwc_filtered'][p4,:]),0)*1e3,
        np.nanmean(um_data['height'],0),
    ...
    ax2.fill_betweenx(np.nanmean(np.squeeze(ra2t_data['height'][p4,:]),0),
        ...
        np.nanmean(np.squeeze(ra2t_data['model_iwc_filtered'][p4,:]),0)*1e3 + np.nanstd(np.squeeze(ra2t_data['model_iwc_filtered'][p4,:]),0)*1e3,
        color = 'lightblue', alpha = 0.15)
    plt.plot(np.nanmean(np.squeeze(ra2t_data['model_iwc_filtered'][p4,:]),0)*1e3 - np.nanstd(np.squeeze(ra2t_data['model_iwc_filtered'][p4,:]),0)*1e3,
        np.nanmean(ra2t_data['height'],0),
        '--', color = 'steelblue', linewidth = 0.5)
    plt.plot(np.nanmean(np.squeeze(ra2t_data['model_iwc_filtered'][p4,:]),0)*1e3 + np.nanstd(np.squeeze(ra2t_data['model_iwc_filtered'][p4,:]),0)*1e3,
        np.nanmean(ra2t_data['height'],0),
    ...
    ax2.fill_betweenx(np.nanmean(np.squeeze(misc_data['height'][p4,:]),0),
        ...
        np.nanmean(np.squeeze(misc_data['model_iwc_filtered'][p4,:]),0)*1e3 + np.nanstd(np.squeeze(misc_data['model_iwc_filtered'][p4,:]),0)*1e3,
        color = 'mediumaquamarine', alpha = 0.15)
    plt.plot(np.nanmean(np.squeeze(misc_data['model_iwc_filtered'][p4,:]),0)*1e3 - np.nanstd(np.squeeze(misc_data['model_iwc_filtered'][p4,:]),0)*1e3,
        np.nanmean(misc_data['height'],0),
        '--', color = 'mediumseagreen', linewidth = 0.5)
    plt.plot(np.nanmean(np.squeeze(misc_data['model_iwc_filtered'][p4,:]),0)*1e3 + np.nanstd(np.squeeze(misc_data['model_iwc_filtered'][p4,:]),0)*1e3,
        np.nanmean(misc_data['height'],0),
    ...
    ax2.fill_betweenx(np.nanmean(np.squeeze(obs_data['height'][p5,:]),0),
        ...
        np.nanmean(fraction0p5,0) + np.nanstd(fraction0p5,0),
        color = 'lightgrey', alpha = 0.5)
    plt.plot(np.nanmean(fraction0p5,0) - np.nanstd(fraction0p5,0),
    ...
    ax2.fill_betweenx(np.nanmean(np.squeeze(um_data['height'][p5,:]),0),
        ...
        np.nanmean(fraction1p5,0) + np.nanstd(fraction1p5,0),
        color = 'blue', alpha = 0.05)
    plt.plot(np.nanmean(fraction1p5,0) - np.nanstd(fraction1p5,0),
        np.nanmean(um_data['height'],0),
        '--', color = 'darkblue', linewidth = 0.5)
    plt.plot(np.nanmean(fraction1p5,0) + np.nanstd(fraction1p5,0),
        np.nanmean(um_data['height'],0),
    ...
    ax2.fill_betweenx(np.nanmean(np.squeeze(ra2t_data['height'][p5,:]),0),
        ...
        np.nanmean(fraction4p5,0) + np.nanstd(fraction4p5,0),
        color = 'lightblue', alpha = 0.15)
    plt.plot(np.nanmean(fraction4p5,0) - np.nanstd(fraction4p5,0),
        np.nanmean(ra2t_data['height'],0),
        '--', color = 'steelblue', linewidth = 0.5)
    plt.plot(np.nanmean(fraction4p5,0) + np.nanstd(fraction4p5,0),
        np.nanmean(ra2t_data['height'],0),
    ...
    ax2.fill_betweenx(np.nanmean(np.squeeze(misc_data['height'][p5,:]),0),
        ...
        np.nanmean(fraction2p5,0) + np.nanstd(fraction2p5,0),
        color = 'mediumaquamarine', alpha = 0.15)
    plt.plot(np.nanmean(fraction2p5,0) - np.nanstd(fraction2p5,0),
        np.nanmean(misc_data['height'],0),
        '--', color = 'mediumseagreen', linewidth = 0.5)
    plt.plot(np.nanmean(fraction2p5,0) + np.nanstd(fraction2p5,0),
        np.nanmean(misc_data['height'],0),
    ...
    ax2.fill_betweenx(np.nanmean(np.squeeze(obs_data['height'][p5,lwc_binlimit:]),0),
        ...
        np.nanmean(np.squeeze(obs_data['lwc_adiabatic_inc_nolwp'][p5,lwc_binlimit:]),0)*1e3 + np.nanstd(np.squeeze(obs_data['lwc_adiabatic_inc_nolwp'][p5,lwc_binlimit:]),0)*1e3,
        color = 'lightgrey', alpha = 0.5)
    plt.plot(np.nanmean(np.squeeze(obs_data['lwc_adiabatic_inc_nolwp'][p5,lwc_binlimit:]),0)*1e3 - np.nanstd(np.squeeze(obs_data['lwc_adiabatic_inc_nolwp'][p5,lwc_binlimit:]),0)*1e3,
        np.nanmean(np.squeeze(obs_data['height'][p5,lwc_binlimit:]),0),
        '--', color = 'k', linewidth = 0.5)
    plt.plot(np.nanmean(np.squeeze(obs_data['lwc_adiabatic_inc_nolwp'][p5,lwc_binlimit:]),0)*1e3 + np.nanstd(np.squeeze(obs_data['lwc_adiabatic_inc_nolwp'][p5,lwc_binlimit:]),0)*1e3,
        np.nanmean(np.squeeze(obs_data['height'][p5,lwc_binlimit:]),0),
    ...
    ax2.fill_betweenx(np.nanmean(np.squeeze(um_data['height'][p5,lwc_binlimit:]),0),
        ...
        np.nanmean(np.squeeze(um_data['model_lwc'][p5,lwc_binlimit:]),0)*1e3 + np.nanstd(np.squeeze(um_data['model_lwc'][p5,lwc_binlimit:]),0)*1e3,
        color = 'blue', alpha = 0.05)
    plt.plot(np.nanmean(np.squeeze(um_data['model_lwc'][p5,lwc_binlimit:]),0)*1e3 - np.nanstd(np.squeeze(um_data['model_lwc'][p5,lwc_binlimit:]),0)*1e3,
        np.nanmean(np.squeeze(um_data['height'][p5,lwc_binlimit:]),0),
        '--', color = 'darkblue', linewidth = 0.5)
    plt.plot(np.nanmean(np.squeeze(um_data['model_lwc'][p5,lwc_binlimit:]),0)*1e3 + np.nanstd(np.squeeze(um_data['model_lwc'][p5,lwc_binlimit:]),0)*1e3,
        np.nanmean(np.squeeze(um_data['height'][p5,lwc_binlimit:]),0),
    ...
    ax2.fill_betweenx(np.nanmean(np.squeeze(ra2t_data['height'][p5,lwc_binlimit:]),0),
        ...
        np.nanmean(np.squeeze(ra2t_data['model_lwc'][p5,lwc_binlimit:]),0)*1e3 + np.nanstd(np.squeeze(ra2t_data['model_lwc'][p5,lwc_binlimit:]),0)*1e3,
        color = 'lightblue', alpha = 0.15)
    plt.plot(np.nanmean(np.squeeze(ra2t_data['model_lwc'][p5,lwc_binlimit:]),0)*1e3 - np.nanstd(np.squeeze(ra2t_data['model_lwc'][p5,lwc_binlimit:]),0)*1e3,
        np.nanmean(np.squeeze(ra2t_data['height'][p5,lwc_binlimit:]),0),
        '--', color = 'steelblue', linewidth = 0.5)
    plt.plot(np.nanmean(np.squeeze(ra2t_data['model_lwc'][p5,lwc_binlimit:]),0)*1e3 + np.nanstd(np.squeeze(ra2t_data['model_lwc'][p5,lwc_binlimit:]),0)*1e3,
        np.nanmean(np.squeeze(ra2t_data['height'][p5,lwc_binlimit:]),0),
    ...
    ax2.fill_betweenx(np.nanmean(np.squeeze(misc_data['height'][p5,lwc_binlimit:]),0),
        ...
        np.nanmean(np.squeeze(misc_data['model_lwc'][p5,lwc_binlimit:]),0)*1e3 + np.nanstd(np.squeeze(misc_data['model_lwc'][p5,lwc_binlimit:]),0)*1e3,
        color = 'mediumaquamarine', alpha = 0.15)
    plt.plot(np.nanmean(np.squeeze(misc_data['model_lwc'][p5,lwc_binlimit:]),0)*1e3 - np.nanstd(np.squeeze(misc_data['model_lwc'][p5,lwc_binlimit:]),0)*1e3,
        np.nanmean(np.squeeze(misc_data['height'][p5,lwc_binlimit:]),0),
        '--', color = 'mediumseagreen', linewidth = 0.5)
    plt.plot(np.nanmean(np.squeeze(misc_data['model_lwc'][p5,lwc_binlimit:]),0)*1e3 + np.nanstd(np.squeeze(misc_data['model_lwc'][p5,lwc_binlimit:]),0)*1e3,
        np.nanmean(np.squeeze(misc_data['height'][p5,lwc_binlimit:]),0),
    ...
    ax2.fill_betweenx(np.nanmean(np.squeeze(obs_data['height'][p5,:]),0),
        ...
        np.nanmean(np.squeeze(obs_data['iwc'][p5,:]),0)*1e3 + np.nanstd(np.squeeze(obs_data['iwc'][p5,:]),0)*1e3,
        color = 'lightgrey', alpha = 0.5)
    plt.plot(np.nanmean(np.squeeze(obs_data['iwc'][p5,:]),0)*1e3 - np.nanstd(np.squeeze(obs_data['iwc'][p5,:]),0)*1e3,
        np.nanmean(np.squeeze(obs_data['height'][p5,:]),0),
        '--', color = 'k', linewidth = 0.5)
    plt.plot(np.nanmean(np.squeeze(obs_data['iwc'][p5,:]),0)*1e3 + np.nanstd(np.squeeze(obs_data['iwc'][p5,:]),0)*1e3,
        np.nanmean(np.squeeze(obs_data['height'][p5,:]),0),
    ...
    ax2.fill_betweenx(np.nanmean(np.squeeze(um_data['height'][p5,:]),0),
        ...
        np.nanmean(np.squeeze(um_data['model_iwc_filtered'][p5,:]),0)*1e3 + np.nanstd(np.squeeze(um_data['model_iwc_filtered'][p5,:]),0)*1e3,
        color = 'blue', alpha = 0.05)
    plt.plot(np.nanmean(np.squeeze(um_data['model_iwc_filtered'][p5,:]),0)*1e3 - np.nanstd(np.squeeze(um_data['model_iwc_filtered'][p5,:]),0)*1e3,
        np.nanmean(um_data['height'],0),
        '--', color = 'darkblue', linewidth = 0.5)
    plt.plot(np.nanmean(np.squeeze(um_data['model_iwc_filtered'][p5,:]),0)*1e3 + np.nanstd(np.squeeze(um_data['model_iwc_filtered'][p5,:]),0)*1e3,
        np.nanmean(um_data['height'],0),
    ...
    ax2.fill_betweenx(np.nanmean(np.squeeze(ra2t_data['height'][p5,:]),0),
        ...
        np.nanmean(np.squeeze(ra2t_data['model_iwc_filtered'][p5,:]),0)*1e3 + np.nanstd(np.squeeze(ra2t_data['model_iwc_filtered'][p5,:]),0)*1e3,
        color = 'lightblue', alpha = 0.15)
    plt.plot(np.nanmean(np.squeeze(ra2t_data['model_iwc_filtered'][p5,:]),0)*1e3 - np.nanstd(np.squeeze(ra2t_data['model_iwc_filtered'][p5,:]),0)*1e3,
        np.nanmean(ra2t_data['height'],0),
        '--', color = 'steelblue', linewidth = 0.5)
    plt.plot(np.nanmean(np.squeeze(ra2t_data['model_iwc_filtered'][p5,:]),0)*1e3 + np.nanstd(np.squeeze(ra2t_data['model_iwc_filtered'][p5,:]),0)*1e3,
        np.nanmean(ra2t_data['height'],0),
    ...
    ax2.fill_betweenx(np.nanmean(np.squeeze(misc_data['height'][p5,:]),0),
        ...
        np.nanmean(np.squeeze(misc_data['model_iwc_filtered'][p5,:]),0)*1e3 + np.nanstd(np.squeeze(misc_data['model_iwc_filtered'][p5,:]),0)*1e3,
        color = 'mediumaquamarine', alpha = 0.15)
    plt.plot(np.nanmean(np.squeeze(misc_data['model_iwc_filtered'][p5,:]),0)*1e3 - np.nanstd(np.squeeze(misc_data['model_iwc_filtered'][p5,:]),0)*1e3,
        np.nanmean(misc_data['height'],0),
        '--', color = 'mediumseagreen', linewidth = 0.5)
    plt.plot(np.nanmean(np.squeeze(misc_data['model_iwc_filtered'][p5,:]),0)*1e3 + np.nanstd(np.squeeze(misc_data['model_iwc_filtered'][p5,:]),0)*1e3,
        np.nanmean(misc_data['height'],0),
...
def plot_BiasCorrelation(obs_data, um_data, misc_data, ifs_data, ra2t_data, doy, obs_switch):
    ...
    um_data['lwc_bias'] = (um_data['model_lwc'][::6,data1['universal_height_UMindex']] - obs_data['lwc_adiabatic_inc_nolwp'][::6,data1['universal_height_UMindex']])*1e3
    misc_data['lwc_bias'] = (misc_data['model_lwc'][::6,data1['universal_height_UMindex']] - obs_data['lwc_adiabatic_inc_nolwp'][::6,data1['universal_height_UMindex']])*1e3
    ifs_data['lwc_bias'] = (ifs_data['model_lwc'][::6,data1['universal_height_UMindex']] - obs_data['lwc_adiabatic_inc_nolwp'][::6,data1['universal_height_UMindex']])*1e3
    ...
    plt.scatter(np.ndarray.flatten(data3['temp_anomalies'][Z1index,:]),np.ndarray.flatten(ifs_data['lwc_bias'][:,2:Z2index[0][-1]]), s = 3, color = '#FFC107')
...
def interpCloudnet(obs_data, month_flag, missing_files, doy):
    ...
    varlist = ['Cv', 'lwc_adiabatic', 'lwc_adiabatic_inc_nolwp', 'iwc', 'lwc']
...
def buildNaNMask(obs_data, month_flag, missing_files, doy):
    ...
    sclwcindex = np.zeros([np.size(obs_data['time'])])
...
def main():
    ...
    obs_switch = 'UM'
    obs_testing_flag = 0 # use ifs input to test obs ceilometer data
    ...
    if platform == 'LAPTOP':
        ...
        if obs_testing_flag == 1:
            cn_ifs_out_dir = ['cloud-fraction-metum-grid/2018/',
...
###################################################################################################################
    ...
    # doy = np.arange(226,253)        ## set DOY for ceilometer testing  (14th Aug to 8th Sep)
...
###################################################################################################################
    ...
    if obs_testing_flag != 1: nanind, nanmask, lwcind, iwcind, wc0ind, lwpind, sclwcind = buildNaNMask(obs_data, month_flag, missing_files, doy)

    if obs_testing_flag != 1:
        varlist_obs = ['Cv', 'lwc_adiabatic_inc_nolwp', 'iwc', 'lwc']
        ...
        # ra2t_data['model_lwp'][lwpind] = np.nan
            ...
            # if varlist_obs[c] == 'lwc_adiabatic_inc_nolwp': cmax = 1e-4
...
###################################################################################################################
    ...
    np.save('obs-v8_Cv_um-qf30', obs_data)
